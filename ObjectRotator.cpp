/******************************************************************************
 *  ObjectRotator.cpp  –  simple “ZERO_DELTA-only” homing
 *
 ...
 *    then moves ZERO_DELTA so the shaft sits at POS_DEG[0]
 *  • All angles in POS_DEG[] and ZERO_DELTA are OUTPUT-SHAFT degrees
 *
 ...
 ******************************************************************************/
 ...
 #include <Stepper.h>v
  ...
  const int   ZERO_DELTA  = 5;                 // ° between released switch and
                                               //    first logical preset
  const float GEAR_MULT   = 9.111111111111111; // motor ° per 1 ° output
 ...
 /* ── I/O pins & timing ─────────────────────────── */
 const int      BTN_PIN      = 2;
 const int      LIM_PIN      = 3;             // active-LOW limit switch
 const uint8_t  LED_PIN      = 13;
 const uint8_t  DEBOUNCE_MS  = 50;
 const uint16_t LONGPRESS_MS = 3000;
 
 /* ── preset list (OUTPUT angles degrees) ─────────────── */
 ...
 /* Crawl to switch, release, jog ZERO_DELTA, resync logic */
 void homeLeftUntilLimit() {
 
   /* crawl left until switch closes */
   while (digitalRead(LIM_PIN) == HIGH)
     motor.step(-10);                       // small negative steps
 
   /* quick lever release (internal constant) */
   motor.step( (long)(RELEASE_DEG * STEPS_PER_DEG_OUT) );
 
   /* jog ZERO_DELTA so we’re at the first preset */
   motor.step( (long)(ZERO_DELTA  * STEPS_PER_DEG_OUT) );
